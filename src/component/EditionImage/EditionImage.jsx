...
export default function EditionImage({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      console.log(event.target.closest(".card__container"));
      event.target.closest(".card__container")['author'] = response.data.author;
      event.target.closest(".card__container")['description'] = response.data.description;
      ...
      <form className="edition__form" onSubmit={handleSubmit} type={type}>
        <div className="edition__image__container">
          ...
          <source src={`http://localhost:5000/${url}`}/>
            ...
            <EditionVideo
              url={
                urlFile !== url ? urlFile : `http://localhost:5000/${urlFile}`
              }
        ...
        {showModifyButton && (
          <div className="edition__button__container">
            ...
            <button
              onClick={() => setEditDescription(inputValue)}
